...
outFileName="dvb_asi_router_2.xml"
...
def createMap():
    ...
    for column in range(numColumns):
        ...
        for row in range(numRows):
            ...
            if outputNr <= cntOutputs:
                print 'create element'+str(outputNr)
                y=str(row+1)
                ...
                for group in subMenuEntries.keys():
           ...
           #         xmlGen.addLine(7,'<menu-item')
           ##         xmlGen.addLine(8,'label="Y'+ str(row+output_start_num+column*numRows)+' - X'+str(inputs+curInputNum)+'"')
           #         xmlGen.addLine(8,'action="de.euromedia_service.coyonet.sendcmd"')
           #         xmlGen.addLine(8,'arg.id="'+engine+'.setcrosspoint_'+ str(row+output_start_num+column*numRows)+'_'+str(inputs+curInputNum)+'"')
           ##         xmlGen.addLine(8,'arg.question="Do you really want to route output Y'+ str(row+output_start_num+column*numRows)+' to input X'+str(inputs+curInputNum)+'?"')
           #         xmlGen.addLine(7,'/>')    
           #     xmlGen.addLine(5,'</menu>')
           # xmlGen.addLine(3,'</menu>') 
           # xmlGen.addLine(3,'</label>')                       
