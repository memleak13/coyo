...
outFileName="gui/maps/dvb_kreuz_asi_2.xml"
mapName="DVB_Kreuz_ASI_2"

previous_Map="DVB_Kreuz_ASI_1"
...
server_ip="172.23.100.43"


#primary sdi - router

col_idx_sigType=2 
col_idx_alt_sigName=1
col_idx_sigName=3
col_idx_inputNum=0
col_idx_outName=7
...
def getSubMenuEntries(dic):
    ...
    for row in range(startOffset,sheet.nrows):
        if not str(sheet.cell(row,col_idx_inputNum).value)=='':   
            if not str(sheet.cell(row,col_idx_sigType).value)=='':
                
                if str(sheet.cell(row,col_idx_sigName).value)<>'':
                    dic[sheet.cell(row,col_idx_sigType).value].append(str(sheet.cell(row,col_idx_sigName).value)+'_'+str(int(sheet.cell(row,col_idx_inputNum).value)))
                else:
                    dic[sheet.cell(row,col_idx_sigType).value].append(str(sheet.cell(row,col_idx_alt_sigName).value)+'_'+str(int(sheet.cell(row,col_idx_inputNum).value))) 
            else:
                
                if str(sheet.cell(row,col_idx_sigName).value)<>'':
                    dic['Unknown group'].append(str(sheet.cell(row,col_idx_sigName).value)+'_'+str(int(sheet.cell(row,col_idx_inputNum).value)))
                else:
                    dic['Unknown group'].append(str(sheet.cell(row,col_idx_alt_sigName).value)+'_'+str(int(sheet.cell(row,col_idx_inputNum).value)))         
    ...
    for column in range(numColumns):
        ...
        for row in range(numRows):
            ...
            if outputNr <= cntInputs:
                ...
                print outputNr
                if (len(outputNames[(outputNr)-1]) > 0) and not (outputNames[(outputNr)-1]=='Änderung') :
                        ...
                        if len(group)>0:
                            ...
                            for sigLabel in subMenuEntries[group]:
                                if len(sigLabel)>0:
                                    ...
                                    if sigLabel.split('_')[0]<>'':
                                        xmlGen.addLine(8,'label="'+sigLabel.split('_')[0]+' ('+sigLabel.split('_')[1]+')"')
                                    else:
                                        xmlGen.addLine(8,'label="nicht vergeben ('+sigLabel.split('_')[1]+')"')
