...
curLevel=2
engine="sdi-router-s"
alarmEngine="alarm.sdi-router-s"
...
outFileName="gui/maps/dvb_sdi_router_bu_2.xml"
mapName="DVB_Kreuz_SDI_BackUp_2"

previous_Map="DVB_Kreuz_SDI_BackUp_1"
...
def createMap():
    ...
    for column in range(numColumns):
        ...
        for row in range(numRows):
            ...
            xmlGen.addLine(2,'input.selectedInput="'+engine+'.level'+str(curLevel)+'.crosspoint.'+ str(outputMapping[outputNr-1])+'.input"' )
            xmlGen.addLine(2,'input.inputService="label.server.'+engine+'.OutputPort.'+str(outputMapping[outputNr-1])+'.name" >')
